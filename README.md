...
- [ ] **设置 Stage 环境变量**：在部署脚本或管道中执行 `export DOTNET_ENVIRONMENT=Stage`（或传递 `--environment Stage` / 设置 `ASPNETCORE_ENVIRONMENT=Stage`），确保 Stage 覆盖与 Key Vault 映射生效；若未设置将继续沿用基础 `appsettings.json` 中的 `UseHmacFallback=true`。详见《阶段 5 联调 Runbook》的 [Stage 环境变量配置](docs/stage5-integration-runbook.md#stage-环境变量配置)。
...
- [ ] **复核诊断端点**：结合 `/api/status` 页面与 Runbook 第 3 章所述的 `/api/metrics`、`/api/audit` 观测，确认 Stage 环境暴露的指标、阶段进度与失败原因均可读取并留痕；使用 `Stage5SmokeTests -- metrics` 可同步检查 `Plugin.StageReadinessFilePath` 指向的共享文件并输出最近成功时间。[《阶段 5 联调 Runbook》指标与审计观测](docs/stage5-integration-runbook.md#L205-L229)
...
6. 当 `TranslationRequest.Text` 超过 `PluginOptions.MaxCharactersPerRequest` 时，无需手动拆分，`TranslationPipeline` 会自动按句段生成分片任务，通过 `OfflineDraftStore` 以共享 Job ID 入队，`DraftReplayService` 背景工作者在速率限制内逐段重试并按顺序合并译文写回草稿，前端只需等待异步通知即可。【F:src/TlaPlugin/Services/TranslationPipeline.cs†L311-L350】【F:src/TlaPlugin/Services/OfflineDraftStore.cs†L61-L152】【F:src/TlaPlugin/Services/DraftReplayService.cs†L94-L155】
