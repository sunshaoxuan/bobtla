...
## 当前状态
项目处于 **阶段 7：多语广播**，在密钥与 OBO 基线之上扩展多语言广播能力：翻译路由会根据附加语言数量调整成本、逐一执行语气重写并写入审计日志，同时 Adaptive Card 以分节形式展示额外译文，Teams 消息扩展可直接渲染新增内容。【F:src/TlaPlugin/Services/TranslationRouter.cs†L71-L135】【F:src/TlaPlugin/Services/AuditLogger.cs†L15-L43】【F:src/TlaPlugin/Teams/MessageExtensionHandler.cs†L21-L49】【F:tests/TlaPlugin.Tests/MessageExtensionHandlerTests.cs†L17-L77】
...
3. **Tab 设置页与术语管理**：在现有多语广播基础上提供 Tab 体验以管理术语上传、附加语言模板与租户统计。
...
- **消息扩展体验**：`MessageExtensionHandler` 输出日文 Adaptive Card，并在预算或速率超限时返回提示卡片；单测验证卡片内容。【F:src/TlaPlugin/Teams/MessageExtensionHandler.cs†L21-L100】【F:tests/TlaPlugin.Tests/MessageExtensionHandlerTests.cs†L17-L154】
...
- **多语广播体验**：`TranslationRouter` 根据附加语言调整预算、逐一重写译文并将结果写入审计，Adaptive Card 与消息扩展渲染出「追加翻訳」分节；单测覆盖卡片内容与审计记录。【F:src/TlaPlugin/Services/TranslationRouter.cs†L71-L135】【F:src/TlaPlugin/Services/AuditLogger.cs†L15-L43】【F:src/TlaPlugin/Teams/MessageExtensionHandler.cs†L21-L49】【F:tests/TlaPlugin.Tests/TranslationRouterTests.cs†L17-L214】【F:tests/TlaPlugin.Tests/MessageExtensionHandlerTests.cs†L17-L154】

### 测试与运行
1. `dotnet restore` – 还原 NuGet 依赖。 
2. `dotnet test` – 执行 xUnit 测试套件，覆盖合规、路由、草稿、缓存限流、OBO 令牌与消息扩展场景。【F:tests/TlaPlugin.Tests/TranslationRouterTests.cs†L1-L202】【F:tests/TlaPlugin.Tests/TokenBrokerTests.cs†L1-L39】【F:tests/TlaPlugin.Tests/MessageExtensionHandlerTests.cs†L17-L154】
3. `dotnet run --project src/TlaPlugin/TlaPlugin.csproj` – 启动本地 API，`POST /api/translate` 接受 `TranslationRequest` 负载返回 Adaptive Card。

> 代码注释以日文撰写，界面返回文案默认使用日文，符合需求文档“代码注释为日文、界面默认日文”的约束。【F:src/TlaPlugin/Services/TranslationRouter.cs†L15-L192】【F:src/TlaPlugin/Teams/MessageExtensionHandler.cs†L9-L100】
