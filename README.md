...
## 当前状态
项目处于 **阶段 8：多模型互联**，在多语广播与安全基线上新增统一的外部模型接入层：`ConfigurableChatModelProvider` 可按配置连接 OpenAI、Claude、Groq、OpenWebUI 或 Ollama，缺省时仍回退到 Mock 实现；Adaptive Card 新增「チャットに挿入」与多语插入按钮，用户可一键将译文写入对话框。【F:src/TlaPlugin/Providers/ConfigurableChatModelProvider.cs†L1-L209】【F:src/TlaPlugin/Services/TranslationRouter.cs†L92-L169】【F:tests/TlaPlugin.Tests/MessageExtensionHandlerTests.cs†L17-L116】
