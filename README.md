...
## 当前状态
项目处于 **阶段 9：前端体验筹备**，在多模型互联的基础上补齐前端所需的数据平面：`/api/configuration`、`/api/glossary`、`/api/audit`、`/api/status` 与新增的 `/api/metrics` 端点分别提供语言、语气模板、用语表、审计、阶段快照及租户级调用统计，并额外公布整体完成度（80%）与前端准备度（数据平面 ✅、界面 ❌、联调 ❌），帮助前端同学掌握当前缺口。使用统计现已同时返回“合规拒绝”“预算不足”“模型错误”“认证失败”等失败原因分布，便于前端仪表盘揭示可靠性。【F:src/TlaPlugin/Program.cs†L42-L98】【F:src/TlaPlugin/Services/ConfigurationSummaryService.cs†L1-L48】【F:src/TlaPlugin/Services/ProjectStatusService.cs†L8-L62】【F:src/TlaPlugin/Services/UsageMetricsService.cs†L1-L123】
